...
using Afonsoft.NewTemplate.Authentication.TwoFactor;
using Afonsoft.NewTemplate.Configuration;
using Afonsoft.NewTemplate.EntityFrameworkCore;
using Afonsoft.NewTemplate.Web.Authentication.JwtBearer;
using Afonsoft.NewTemplate.Web.Configuration;

namespace Afonsoft.NewTemplate.Web
{
    [DependsOn(
        typeof(NewTemplateApplicationModule),
        typeof(NewTemplateEntityFrameworkCoreModule),
        typeof(AbpAspNetZeroCoreWebModule),
        typeof(AbpAspNetCoreSignalRModule),
        typeof(AbpRedisCacheModule), //AbpRedisCacheModule dependency (and Abp.RedisCache nuget package) can be removed if not using Redis cache
        typeof(AbpHangfireAspNetCoreModule) //AbpHangfireModule dependency (and Abp.Hangfire.AspNetCore nuget package) can be removed if not using Hangfire
    )]
    public class NewTemplateWebCoreModule : AbpModule
    {
        private readonly IWebHostEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public NewTemplateWebCoreModule(IWebHostEnvironment env)
        {
            _env = env;
        ...
        public override void PreInitialize()
        {
            ...
            Configuration.Modules.AbpAspNetCore()
                .CreateControllersForAppServices(
                    typeof(NewTemplateApplicationModule).GetAssembly()
                );
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(NewTemplateWebCoreModule).GetAssembly());
        }
        ...
        public override void PostInitialize()
        {
            ...
            IocManager.Resolve<ApplicationPartManager>()
                .AddApplicationPartsIfNotAddedBefore(typeof(NewTemplateWebCoreModule).Assembly);
        }
